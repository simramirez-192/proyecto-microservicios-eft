...
class GlobalExceptionHandlerTest {
    ...
    void testManejarRuntimeException_conMensajeNoEncontrado_retorna404() {
        RuntimeException ex = new RuntimeException("El registro no encontrado en la base de datos");
    ...
    @Test
    void testManejarRuntimeException_conMensajeNoEncontrada_retorna400() {
        RuntimeException ex = new RuntimeException("Categoria no encontrada con id: 99");

        ResponseEntity<Map<String, Object>> respuesta = handler.manejarRuntimeException(ex);

        assertEquals(HttpStatus.BAD_REQUEST, respuesta.getStatusCode());
        assertEquals(400, respuesta.getBody().get("status"));
        assertTrue(respuesta.getBody().get("mensaje").toString().contains("no encontrada"));
    }

    @Test
    void testManejarRuntimeException_conMensajeNoExiste_retorna404() {
        RuntimeException ex = new RuntimeException("El registro no existe en la base de datos");
    ...
    @Test
    void testManejarRuntimeException_conMensajeDeNegocio_retorna400() {
    ...
    @Test
    void testManejarRuntimeException_conMensajeNull_retorna400ConMensajePorDefecto() {
    ...
    @Test
    void testManejarValidacion_conDosErrores_retorna400ConMapaDeErrores() throws Exception {
        MethodParameter parameter = mock(MethodParameter.class);
        BindingResult bindingResult = mock(BindingResult.class);

        FieldError error1 = new FieldError("requestDTO", "nombre", "El nombre es obligatorio");
        FieldError error2 = new FieldError("requestDTO", "descripcion", "La descripcion es obligatoria");
    ...
    @SuppressWarnings("unchecked")
    @Test
    void testManejarValidacion_conUnSoloError_retorna400() throws Exception {
        MethodParameter parameter = mock(MethodParameter.class);
        BindingResult bindingResult = mock(BindingResult.class);

        FieldError error = new FieldError("requestDTO", "nombre", "El nombre es obligatorio");
